...
const AdmissionResult = () => {
  ...
  /* ================= FETCH DATA ================= */
  const { data: resultsData = [], refetch, isLoading } = useQuery({
    queryKey: ["admission-results"],
    queryFn: async () => {
      const res = await axiosSecure.get("/admission-results");
      return res.data;
    },
  });
  ...
  const handleAddResult = async (data) => {
    ...
    try {
      const res = await axiosSecure.post("/admission-results", formData, {
      ...
      if (res.data.insertedId) {
        ...
        alert("Admission result uploaded successfully");
      }
    } catch (err) {
      alert(err.message);
    }
  ...
  const handleDelete = async (id) => {
    try {
      await axiosSecure.delete(`/admission-results/${id}`);
      refetch();
  ...
  const handleEdit = async (data) => {
    ...
    try {
      await axiosSecure.patch(`/admission-results/${isEditing._id}`, formData);
      refetch();
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">Admission Result Management</h1>
      ...
      <dialog ref={modalRef} className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg mb-4">Add Admission Result</h3>

          <form onSubmit={handleSubmit(handleAddResult)} className="space-y-4">
            <input
              type="text"
              placeholder="Title"
      ...
      <dialog ref={modalRefedited} className="modal">
        <div className="modal-box">
          <h3 className="font-bold text-lg mb-4">Edit Admission Result</h3>
