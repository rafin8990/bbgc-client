...
const AdmissionResultpage = () => {
  ...
  const { data: admissionResults = [], isLoading } = useQuery({
  ...
  return (
    <div className="min-h-screen bg-[#050b1e] text-white px-4 md:px-6 py-16">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Title */}
        <h1 className="text-3xl font-bold mb-8">🎓 Admission Results</h1>

        {isLoading ? (
          <p className="text-center text-gray-400">Loading admission results...</p>
        ) : admissionResults.length === 0 ? (
          <p className="text-center text-gray-400">No admission results found.</p>
        ) : (
          <>
            {/* ================= MOBILE VIEW ================= */}
            <div className="grid gap-4 md:hidden">
              {admissionResults.map((item, i) => (
                <div
                  key={item._id}
